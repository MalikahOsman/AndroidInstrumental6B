...
import com.example.osman.restaurantmsandroid.reposities.PosRepository;
...
public class PosRepositoryImpl extends SQLiteOpenHelper implements PosRepository {
    ...
    public POS_Cashier findById(Long id) {
        ...
        if(cursor.moveToFirst()){
            final POS_Cashier Waiter = new POS_Cashier.Builder()
    ...
    public Set<POS_Cashier> findAll() {
        SQLiteDatabase db = this.getReadableDatabase();
        Set<POS_Cashier> POS_Cashier = new HashSet<>();
        open();

        Cursor cursor = db.query(TABLE_NAME, null,null,null,null,null,null);
        if (cursor.moveToFirst()){
            do{
                final POS_Cashier setting = new POS_Cashier.Builder()
                        .id(cursor.getLong(cursor.getColumnIndex(COLUMN_ID)))
                        .userName(cursor.getString(cursor.getColumnIndex(COLUMN_USER_NAME)))
                        .password(cursor.getString(cursor.getColumnIndex(COLUMN_PASSWORD)))
                        .empNum(cursor.getString(cursor.getColumnIndex(COLUMN_EMPLOYEE_NUMBER)))
