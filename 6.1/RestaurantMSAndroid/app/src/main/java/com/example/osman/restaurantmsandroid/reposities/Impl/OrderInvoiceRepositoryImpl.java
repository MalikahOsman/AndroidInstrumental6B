 ...
 */
public class OrderInvoiceRepositoryImpl extends SQLiteOpenHelper implements com.example.osman.restaurantmsandroid.reposities.OrderInvoiceRepository {
    ...
    public OrderInvoice findById(Long id) {
        ...
        if(cursor.moveToFirst()){
            final OrderInvoice OrderInvoice = new OrderInvoice.Builder()
    ...
    public Set<OrderInvoice> findAll() {
        SQLiteDatabase db = this.getReadableDatabase();
        Set<OrderInvoice> OrderInvoice = new HashSet<>();
        open();

        Cursor cursor = db.query(TABLE_NAME, null,null,null,null,null,null);
        if (cursor.moveToFirst()){
            do{
                final OrderInvoice setting = new OrderInvoice.Builder()
                        .id(cursor.getLong(cursor.getColumnIndex(COLUMN_ID)))
                        .invoiceDate(cursor.getString(cursor.getColumnIndex(COLUMN_INVOICE_DATE)))
                        .orderAmount(cursor.getDouble(cursor.getColumnIndex(COLUMN_ORDER_AMOUNT)))
                        .invoiceStatus(cursor.getString(cursor.getColumnIndex(COLUMN_INVOICE_STATUS)))
                        .build();
                OrderInvoice.add(setting);
